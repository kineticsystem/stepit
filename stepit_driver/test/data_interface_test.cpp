...
#include <stepit_driver/data_interface.h>
#include <stepit_driver/data_utils.h>
#include <stepit_driver/serial_exception.h>
...
{
  MockSerial mock;
  stepit_driver::DataInterface data_interface{ &mock };
  ...
  ASSERT_THAT(stepit_driver::data_utils::to_hex(actual_frame), stepit_driver::data_utils::to_hex(expected_frame));
}
...
{
  MockSerial mock;
  stepit_driver::DataInterface cmd_interface{ &mock };
  ...
  EXPECT_THROW(
      {
        ...
        catch (const stepit_driver::SerialException& e)
        {
          EXPECT_STREQ("SerialException: error writing to the serial port.", e.what());
          throw;
        }
      },
      stepit_driver::SerialException);
}
...
TEST(command_interface, read)
{
  ...
  ASSERT_THAT(stepit_driver::data_utils::to_hex(actual_data), stepit_driver::data_utils::to_hex(expected_data));
}
