...
constexpr byte MOVE_TO_CMD = 0x71;             // Move motors to a given position.
 ...
 */
void moveMotors(byte requestId, DataBuffer* cmd)
{
  while (cmd->getSize() > 0)
  {
    byte motorId = cmd->removeByte(Location::FRONT);
    long position = radiansToSteps(cmd->removeFloat(Location::FRONT));
    float speed = fabs(radiansToSteps(cmd->removeFloat(Location::FRONT)));
